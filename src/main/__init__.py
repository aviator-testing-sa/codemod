...
from
 user.current_user import CurrentUser
from
 user.current_user import Anonymous
...
get('ENVIRONMENT', 'dev')
    ...
    # monkey patch some easy-to-use
 routes
    app.get = functools.
partial(app.route, methods=['GET'])

    app.post = functools.partial
(app.route, methods=['POST'])

    app.delete = functools.partial(
app.route, methods=['DELETE'])
    
app.put = functools.partial(app
.route, methods=['PUT'])
...
'''
Initialize models. This import
 makes sure all the models
are defined and parse
d by SQL Alchemy.
'''
import
 schema
...
'''
Initialize all views so that
 all the route are set correctly.
'''
